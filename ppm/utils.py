...
from globals import CONFIG, COUNTRY_MAP, SUB_LAND
...
def correct_manually(df, name):
    ...
    corrections_fn = glob.ref_data('manual_corrections.csv')
...
def fill_geoposition(df, use_saved_locations=False, saved_only=False):
    ...
    if use_saved_locations:
        locs = pd.read_csv(glob.ref_data('parsed_locations.csv'), index_col=[0, 1])
